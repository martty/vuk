...
namespace vuk {
	...
	public:
		...
		/// @brief Update a sampler at a specific index
		/// @param index The index to update
		/// @param sampler The sampler to set
		void set(uint32_t index, Sampler sampler) {
			assert(is_allocated(index) && "Index not allocated");
			assert(binding_indices[static_cast<size_t>(DescriptorType::eSampler)] != ~0U && "Sampler binding not configured");
		...
		/// @brief Update a sampled image at a specific index
		/// @param index The index to update
		/// @param image_view The image view to set
		/// @param layout The image layout
		void set(uint32_t index, ImageView image_view, ImageLayout layout) {
			assert(is_allocated(index) && "Index not allocated");
			assert(binding_indices[static_cast<size_t>(DescriptorType::eSampledImage)] != ~0U && "Sampled image binding not configured");
			...
			db.image.dii.imageLayout = static_cast<VkImageLayout>(layout);
			add_write(binding_indices[static_cast<size_t>(DescriptorType::eSampledImage)], index, db);
		}

		/// @brief Update a combined image sampler at a specific index
		/// @param index The index to update
		/// @param image_view The image view to set
		/// @param sampler The sampler to use
		/// @param layout The image layout
		void set(uint32_t index, ImageView image_view, Sampler sampler, ImageLayout layout) {
			assert(is_allocated(index) && "Index not allocated");
			assert(binding_indices[static_cast<size_t>(DescriptorType::eCombinedImageSampler)] != ~0U && "Combined image sampler binding not configured");
		...
		/// @brief Update a storage image at a specific index
		/// @param index The index to update
		/// @param image_view The image view to set
		void set(uint32_t index, ImageView image_view) {
			assert(is_allocated(index) && "Index not allocated");
			assert(binding_indices[static_cast<size_t>(DescriptorType::eStorageImage)] != ~0U && "Storage image binding not configured");
			...
			db.type = DescriptorType::eStorageImage;
			db.image.set_image_view(image_view);
			db.image.dii.imageLayout = VK_IMAGE_LAYOUT_GENERAL;
			add_write(binding_indices[static_cast<size_t>(DescriptorType::eStorageImage)], index, db);
		}

		/// @brief Update a uniform buffer at a specific index
		/// @param index The index to update
		/// @param buffer The buffer to set
		void set_uniform_buffer(uint32_t index, Buffer buffer) {
			assert(is_allocated(index) && "Index not allocated");
			assert(binding_indices[static_cast<size_t>(DescriptorType::eUniformBuffer)] != ~0U && "Uniform buffer binding not configured");
		...
		/// @brief Update a storage buffer at a specific index
		/// @param index The index to update
		/// @param buffer The buffer to set
		void set_storage_buffer(uint32_t index, Buffer buffer) {
			assert(is_allocated(index) && "Index not allocated");
			assert(binding_indices[static_cast<size_t>(DescriptorType::eStorageBuffer)] != ~0U && "Storage buffer binding not configured");
		...
		/// @brief Update an acceleration structure at a specific index
		/// @param index The index to update
		/// @param accel_struct The acceleration structure to set
		void set(uint32_t index, VkAccelerationStructureKHR accel_struct) {
			assert(is_allocated(index) && "Index not allocated");
			assert(binding_indices[static_cast<size_t>(DescriptorType::eAccelerationStructureKHR)] != ~0U && "Acceleration structure binding not configured");
