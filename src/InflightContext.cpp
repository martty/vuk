...
vuk::InflightContext::InflightContext(Context& ctx, size_t absolute_frame, std::lock_guard<std::mutex>&& recycle_guard) :
	...
	switch (remainder) {
	case 0:
		ctx.impl->pipeline_cache.collect(absolute_frame, cache_collection_frequency); break;
	case 1:
		ctx.impl->compute_pipeline_cache.collect(absolute_frame, cache_collection_frequency); break;
	case 2:
		ctx.impl->renderpass_cache.collect(absolute_frame, cache_collection_frequency); break;
	/*case 3:
	...
	case 4:
		ctx.impl->pipeline_layouts.collect(absolute_frame, cache_collection_frequency); break;
	case 5:
		ctx.impl->pipelinebase_cache.collect(absolute_frame, cache_collection_frequency); break;
	case 6:
		ctx.impl->compute_pipelinebase_cache.collect(absolute_frame, cache_collection_frequency); break;
	}
}
