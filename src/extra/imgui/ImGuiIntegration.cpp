...
namespace vuk::extra {
	...
	ImGuiData ImGui_ImplVuk_Init(Allocator& allocator) {
		...
		ImGuiData data;
		auto font_ia =
			ImageAttachment::from_preset(ImageAttachment::Preset::eMap2D, Format::eR8G8B8A8Unorm, Extent3D{ (unsigned)width, (unsigned)height, 1u }, Samples::e1);
	...
	Value<ImageAttachment> ImGui_ImplVuk_Render(Allocator& allocator, Value<ImageAttachment> target, ImGuiData& data) {
		...
		auto reset_render_state = [](const ImGuiData& data, CommandBuffer& command_buffer, ImDrawData* draw_data, Buffer vertex, Buffer index) {
			...
			pc.is_srgb = 0;
			command_buffer.push_constants(ShaderStageFlagBits::eVertex | ShaderStageFlagBits::eFragment, 0, pc);
		};
								  ...
								  if (pcmd->UserCallback != nullptr) {
									  ...
									  if (clip_rect.x < fb_width && clip_rect.y < fb_height && clip_rect.z >= 0.0f && clip_rect.w >= 0.0f) {
										  ...
										  if (pcmd->TextureId) {
											  ...
											  const auto &image = sis[ia_index];
											  is_srgb = is_format_srgb(image.ia.format);								  
											  command_buffer.bind_image(0, 0, image.ia).bind_sampler(0, 0, image.sci);
										  }

										  command_buffer.push_constants(ShaderStageFlagBits::eVertex | ShaderStageFlagBits::eFragment, offsetof(PushConstants, is_srgb), is_srgb);
										  // Draw
